...
class ImprovedFastWeatherCrawler:
    ...
    def validate_province_codes(self):
        """校验省份代码格式（中国气象局省份代码应为9位数字）"""
        invalid_codes = []
        for province, info in self.provinces.items():
            code = info.get('code', '')
            if not (isinstance(code, str) and len(code) == 9 and code.isdigit()):
                invalid_codes.append(f"{province}（代码：{code}）")
        if invalid_codes:
            logging.warning(f"⚠️ 以下省份代码格式无效（需9位数字）：{', '.join(invalid_codes)}")
        else:
            logging.info("✅ 所有省份代码格式校验通过")
    ...
    def get_weather_with_retry(self, province_name, province_info):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.info(f"第{attempt + 1}次尝试获取 {province_name} 天气数据（URL：{url}）")
                ...
                logging.warning(f"⚠️ {province_name} HTTP错误: {status_code}")
                self.request_stats['failed_requests'] += 1

                if status_code == 404:
                    logging.warning(f"⚠️ {province_name} 页面不存在（HTTP 404），尝试其他数据源")
                    # 尝试切换到备用数据源
                    alt_url = f"https://tianqi.2345.com/wea_history/{code}.htm"
                    logging.info(f"尝试备用数据源: {alt_url}")
                    try:
                        response = self.session.get(alt_url, headers=headers, timeout=15)
                        if response.status_code == 200:
                            logging.info(f"✅ {province_name} 备用数据源请求成功")
                            return self.parse_weather_data(response.text, province_name)
                    except:
                        pass
                ...
                if attempt < max_retries - 1:
                    ...
                    logging.warning(f"❌ {province_name} 所有重试失败，返回备用数据")
                ...
                if attempt < max_retries - 1:
                    ...
                    logging.warning(f"❌ {province_name} 超时重试失败，返回备用数据")
                    return self.get_fallback_weather(province_name)
            except Exception as e:
                ...
                if attempt < max_retries - 1:
                    ...
                    logging.warning(f"❌ {province_name} 异常重试失败，返回备用数据")
                    return self.get_fallback_weather(province_name)

        logging.warning(f"❌ {province_name} 完成{max_retries}次重试均失败，返回备用数据")
        return self.get_fallback_weather(province_name)
    ...
    def update_all_provinces_safely(self):
        ...
        logging.info(
            f"📊 更新模式: {'增量更新' if self.incremental_update else '全量更新'}, 间隔: {self.update_interval}秒")
        ...
        # 显示请求统计
        success_rate = (self.request_stats['total_requests'] - self.request_stats['failed_requests']) / \
                       self.request_stats['total_requests'] * 100 if self.request_stats['total_requests'] > 0 else 0
        logging.info(
            f"📊 请求统计: 总计{self.request_stats['total_requests']}次, 失败{self.request_stats['failed_requests']}次, 成功率{success_rate:.1f}%")

        logging.info(
            f"📊 更新完成: 成功{success_count}省, 跳过{skipped_count}省, 记录{total_records}条, 耗时{total_time:.1f}秒")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 验证省份代码
        crawler.validate_province_codes()
